...
void PM_PlayStepSound( int step, float fvol )
{
	...
	if (pmove->bindeep)
	{
		...
		default:
			...
			{
				// right foot
			case 0:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_astep1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	break;
			case 1:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_astep3.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	break;
				// left foot
			case 2:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_astep2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	break;
			case 3:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_astep4.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	break;
			}
			break;
		case STEP_SLOSH:
			switch (irand)
		...
		else
		{
			...
			if (forward != 0)
			{
				...
				if (pmove->flTimeLadderPunch <= 0)
				{
					...
					if (pmove->leftright != -1)
					{
						switch (pmove->RandomLong(0, 3))
						{
							// right foot
						case 0:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_pain2.wav", 0.6, ATTN_NORM, 0, PITCH_NORM);	break;
						case 1:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_pain4.wav", 0.6, ATTN_NORM, 0, PITCH_NORM);	break;
							// left foot
						case 2:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_pain5.wav", 0.6, ATTN_NORM, 0, PITCH_NORM);	break;
						case 3:	pmove->PM_PlaySound(CHAN_BODY, "player/pl_pain6.wav", 0.6, ATTN_NORM, 0, PITCH_NORM);	break;
						}
					}
