...
void CTripmine::ItemPostFrame(void)
{
	...
	if (placing == TRUE)
	{
		fakeframe += 1;
		//StudioFrameAdvance();
		ALERT(at_console, "I hurt people %f\n", fakeframe);
		Placing();
	}

	if ((m_pPlayer->pev->button & IN_ATTACK) && placing == FALSE)
	{
		SendWeaponAnim(TRIPMINE_ARM2);
		Placing();
	}
	else if (placing == FALSE && !(m_pPlayer->pev->button & IN_ATTACK))
	{
		WeaponIdle();
	}
	if (ShouldWeaponIdle())
