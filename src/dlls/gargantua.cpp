...
// Unused Garg Spear
//=========================================================
class CGargSpear : public CBaseAnimating
{
public:
	void Spawn(void);
...
LINK_ENTITY_TO_CLASS(gargspear, CGargSpear);

TYPEDESCRIPTION	CGargSpear::m_SaveData[] =
{
	DEFINE_FIELD(CGargSpear, m_maxFrame, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CGargSpear, CBaseEntity);

void CGargSpear::Spawn(void)
{
...
void CGargSpear::Shoot(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity)
{
	CGargSpear* pSpit = GetClassPtr((CGargSpear*)NULL);
	pSpit->Spawn();
	...
	pSpit->SetThink(&CGargSpear::Animate);
	pSpit->pev->nextthink = gpGlobals->time + 0.1;
}

void CGargSpear::Touch(CBaseEntity* pOther)
{
	TraceResult tr;
	int		iPitch;
	SetTouch(NULL);
	...
	if (!pOther->pev->takedamage)
	{
		...
		SetThink(&CGargSpear::SUB_Remove);
...
void CGargantua :: FlameCreate( void )
{
	...
	CGargSpear::Shoot(pev, vecSpitOffset, vecSpitDir * 900);
}
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case GARG_AE_NEEDLE:
	{
		...
		CGargSpear::Shoot(pev, vecArmPos, vecSpitDir * 900);
		CGargSpear::Shoot(pev, vecArm2Pos, vecSpit2Dir * 900);
	}
	break;
